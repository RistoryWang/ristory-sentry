...
public class SentryClient {
    ...
    /**
     * Add PGLevel : hard code
     * @param eventBuilder
     * @return
     */
    private EventBuilder addPGLevel(EventBuilder eventBuilder){
        if(eventBuilder.getEvent().getLevel().equals(Event.Level.FATAL)){
            eventBuilder.withTag("pg_level", "fatal");
            eventBuilder.withLevel(Event.Level.FATAL);
            return eventBuilder;
        }
        if(eventBuilder.getEvent().getLevel().equals(Event.Level.EMER)){
            eventBuilder.withTag("pg_level", "emer");
            eventBuilder.withLevel(Event.Level.ERROR);
            return eventBuilder;
        }
        if(eventBuilder.getEvent().getLevel().equals(Event.Level.ERROR)){
            eventBuilder.withTag("pg_level", "error");
            eventBuilder.withLevel(Event.Level.ERROR);
            return eventBuilder;
        }
        if(eventBuilder.getEvent().getLevel().equals(Event.Level.BEAT)){
            eventBuilder.withTag("pg_level", "beat");
            eventBuilder.withLevel(Event.Level.WARNING);
            return eventBuilder;
        }
        if(eventBuilder.getEvent().getLevel().equals(Event.Level.BIZ)){
            eventBuilder.withTag("pg_level", "biz");
            eventBuilder.withLevel(Event.Level.WARNING);
            return eventBuilder;
        }
        if(eventBuilder.getEvent().getLevel().equals(Event.Level.WARNING)){
            eventBuilder.withTag("pg_level", "warning");
            eventBuilder.withLevel(Event.Level.WARNING);
            return eventBuilder;
        }
        if(eventBuilder.getEvent().getLevel().equals(Event.Level.DIAG)){
            eventBuilder.withTag("pg_level", "diag");
            eventBuilder.withLevel(Event.Level.INFO);
            return eventBuilder;
        }
        if(eventBuilder.getEvent().getLevel().equals(Event.Level.INFO)){
            eventBuilder.withTag("pg_level", "info");
            eventBuilder.withLevel(Event.Level.INFO);
            return eventBuilder;
        }
        if(eventBuilder.getEvent().getLevel().equals(Event.Level.NOTICE)){
            eventBuilder.withTag("pg_level", "notice");
            eventBuilder.withLevel(Event.Level.DEBUG);
            return eventBuilder;
        }
        if(eventBuilder.getEvent().getLevel().equals(Event.Level.DEBUG)){
            eventBuilder.withTag("pg_level", "debug");
            eventBuilder.withLevel(Event.Level.DEBUG);
            return eventBuilder;
        }
        if(eventBuilder.getEvent().getLevel().equals(Event.Level.VERBOSE)){
            eventBuilder.withTag("pg_level", "verbose");
            eventBuilder.withLevel(Event.Level.DEBUG);
            return eventBuilder;
        }
        return eventBuilder;
